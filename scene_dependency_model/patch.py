...
class CommitAggregator:
    def __init__(self, file_transformer):
        self.tokenizer = RobertaTokenizer.from_pretrained("pdbert-base")
    ...
    def transform(self, diff_list):
        ...
        for diff in diff_list:
            added_code = patch_before.get_code_version(diff=diff, added_version=True)
            deleted_code = patch_before.get_code_version(diff=diff, added_version=False)

            code = added_code + self.tokenizer.sep_token + deleted_code
            input_ids, mask = patch_before.get_input_and_mask(self.tokenizer, [code])
...
def do_train(args):
    ...
    patch_data, label_data, url_data = patch_before.get_data(dataset_name)
...
if __name__ == '__main__':
    class args:
        def __init__(self,dataset_path,model_path,finetune_model_path,train_prob_path,test_prob_path):
            self.dataset_path = dataset_path
            self.model_path = model_path
            self.finetune_model_path = finetune_model_path
            self.train_prob_path = train_prob_path
            self.test_prob_path = test_prob_path

    args.dataset_path='data/message.json'
    args.model_path='data/model/patch_finetuned_model.sav'
    args.finetune_model_path='data/model/patch_vulfixminer_finetuned_model.sav'
    args.train_prob_path='data/probs/patch_train_prob.txt'
    args.test_prob_path='data/probs/patch_test_prob.txt'
