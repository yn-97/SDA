...
def infer_message_classifier(config_dict):
    ...
    precision, recall, f1, auc, y_train_probs = message_classifier.predict_test_data(model, training_generator, device, need_probs=True)
    ...
    precision, recall, f1, auc, y_test_probs = message_classifier.predict_test_data(model, testing_generator, device, need_probs=True)
...
def commit_classifier_ensemble_new(config_dict):
    ...
    for id in id_train:
        ...
        if url in url_to_mes_train_prob:
            X_train.append([url_to_mes_train_prob[url], url_to_patch_train_prob[url]])
            Y_train.append(id_to_train_label[id])
    
    for id in id_test:
        url = id_to_test_url[id]
        if url in url_to_mes_test_prob:
            X_test.append([url_to_mes_test_prob[url], url_to_patch_test_prob[url]])
    ...
    auc = metrics.accuracy_score(y_pred=y_pred, y_true=Y_test)
    pre = metrics.precision_score(y_pred=y_pred, y_true=Y_test)
    reca = metrics.recall_score(y_pred=y_pred, y_true=Y_test)


    print("auc Commit ensemble Classifier: {}".format(auc))
    print("pre Commit ensemble Classifier: {}".format(pre))
    print("reca Commit ensemble Classifier: {}".format(reca))
...
if __name__ == '__main__':
    ...
    args.config_file='tf_dataset.conf'
